...
const Notice = () => {
  ...
  const FuncValidation = () => {
    // name 
    if (noticeHeading === "" || !noticeHeading) {
      setIsValidNameRequired(true)
    }
    else {
    }
    // description
    if (noticeDescription === "" || !noticeDescription) {
  ...
  const MyNoticeGetAllApi = async () => {
    setLoader(true)
    try {
      const response = await NoticeGetAllApi(searchKey);
  ...
  const MyNoticeGetByIdApi = async (id) => {
    ...
    try {
      ...
      if (response?.status === 200) {
        ...
        setNoticeHeading(response?.data?.Notices?.noticeTitle)
        setNoticeDescription(response?.data?.Notices?.noticeDescription)
        setLoader(false)
      } else {
        toast.error(response?.data?.msg);
      }
                ...
                <div className="offcanvas-header d-block for-my-display">
                  ...
                  <div className="delete-section mt-5">
                    <div className="bg-container">
                      ...
                      <div className='button-position'  >
                        <button type="button" className="btn btn-outline-primary  mt-4 mb my-button112233 " data-bs-dismiss="offcanvas" aria-label="Close" style={{ fontSize: '14px' }} onClick={MyNoticeGetAllApi}>Continue</button>
