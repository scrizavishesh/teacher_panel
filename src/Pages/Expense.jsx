...
import { ExpensePutApi } from '../Utils/Apis'
...
const Expense = () => {
  ...
  const [myExpenseCategoryData, setMyExpenseCategoryData] = useState([])
  ...
  const MyIncomeGetAllApi = async () => {
    setLoader(true)
    try {
      const response = await IncomeCategorygetAllApi(expense);
...
// --------------------------- 
  ...
  const MyIncomePostApi = async () => {
    ...
    setLoader(true)
    try {
      const response = await ExpensePostApi(formData);
      console.log('psotttttt api',response)
      if (response?.status === 200) {
  ...
  // Get All api 
  const MyExpense2GetAllApi = async () => {
    setLoader(true)
    try {
      const response = await ExpenseAllApi(expense);
      console.log('my Expense all  data000000', response);
      if (response?.status === 200) {
        toast.success(response?.data?.message)
        setMyExpenseCategoryData(response?.data?.transaction)
        setLoader(false)
      } else {
        toast.error(response?.data?.classes?.message);
      }
   ...
   // Delete api
   const MyEpenseDelApi = async (id) => {
    setLoader(true)

    try {
      const response = await ExpenseDeleteApi(id);
    ...
    //  Get by id 
    const MyExpenseGetById = async (id) => {
      setIdForUpdate(id)
      setLoader(true)
      try {
        const response = await ExpenseGetByIdApi(id);
   ...
   const MyIncomeCategoryPutApi = async () => {
    ...
    try {
      ...
      const response = await ExpensePutApi(IdForUpdate, formData);
      ...
      <div className="container-fluid main-body p-3">
        ...
        <div className='d-flex justify-content-between for-dislay-direction'>
          <div className="breadCrum ms-2">
            <nav style={{ '--bs-breadcrumb-divider': "'>'" }} aria-label="breadcrumb">
              <ol className="breadcrumb ms-2">
                ...
                <li className="breadcrumb-item breadcrum-li heading-14" ><Link href="#">Expenses</Link></li>
              </ol>
            </nav>
          </div>
          <div className='d-flex g-1 for-media-query'>
            ...
            <Link type="button" className="btn btn-success heading-16 my-own-button me-3 " data-bs-toggle="offcanvas" data-bs-target="#offcanvasRight" aria-controls="offcanvasRight" to={''}>+ Add Expenses</Link>
          </div>
        </div>
        <h5 className='ms-3 mb-2 margin-minus22 heading-16' style={{ marginTop: '-22px' }}>Expenses Details</h5>
        ...
        <div className="main-content-conatainer pt-1 ">
          ...
          <div className="table-container px-3 table-responsive">
            <table className="table table-sm table-striped">
              ...
              {
                  myExpenseCategoryData.map((item, index) => (
                      ...
                      <td className=' greyText' >
                        <div className="dropdown my-button-show">
                          ...
                          <ul className="dropdown-menu anchor-color heading-14">
                            <li><Link className="dropdown-item" to={''} data-bs-toggle="offcanvas" data-bs-target="#offcanvasRight1234" aria-controls="offcanvasRight1234" onClick={(e) => MyExpenseGetById(item.id)} >Edit</Link></li>
              ...
              <>
                ...
                <div className="offcanvas-body pt-0">
                  <div class="mb-3">
                    ...
                    <input type="date" class="form-control form-control-sm" onChange={(e)=> setDate(e.target.value)} id="exampleFormControlInput1" placeholder="Select Title" />
                  </div>
                  <div class="mb-3">
                    <label for="exampleFormControlInput1" class="form-label heading-16">Amount</label>
                    <input type="text" class="form-control form-control-sm"  onChange={(e)=> setAmount(e.target.value)} id="exampleFormControlInput1" placeholder="Enter Amount" />
                  </div>


                  <div className="mb-1  ">
                    <label for="exampleFormControlInput1" className="form-label   heading-16">Expense Category</label>
                  ...
                  <div className='my-button11 '>
                    <button type="button" className="btn btn-outline-success heading-16 btn-bgAndColor" onClick={MyIncomePostApi}>Add Expense</button>
            ...
            showadd && (
              <>
                ...
                <div className="offcanvas-body pt-0">
                  <div class="mb-3">
                    <label for="exampleFormControlInput1" class="form-label heading-16">Date</label>
                    <input type="date" class="form-control form-control-sm" value={date} onChange={(e)=> setDate(e.target.value)} id="exampleFormControlInput1" placeholder="Select Title" />
                  </div>
                  <div class="mb-3">
                    <label for="exampleFormControlInput1" class="form-label heading-16">Amount</label>
                    <input type="text" class="form-control form-control-sm"  value={amount} onChange={(e)=> setAmount(e.target.value)} id="exampleFormControlInput1" placeholder="Enter Amount" />
                  </div>
                  ...
                  <div className='my-button11 '>
                    <button type="button" className="btn btn-outline-success heading-16 btn-bgAndColor" onClick={MyIncomeCategoryPutApi}>Update Expense</button>
                    <button type="button" className="btn btn-outline-success heading-16">Cancel</button>
                  </div>
                </div>
                  ...
                  <div className="sure-main-container mt-4">
                    <div className="sure-container">
                      ...
                      <div className="form-check mt-1">
                        <input className="form-check-input my-form-check-input" onChange={(e)=> setForDelete(true)} type="checkbox" value="" id="flexCheckDefault" />
